---
title: "Decision tree"
subtitle: "Series 2.1 - data collection and pre-processing (part 1)"
...
The assay data was added after I went through the rest of the data_preprocessing, as I thought to check on the confidence scores for assays used in the final data collected (to assess assay-to-target relationships). This [link from ChEMBL](https://chembl.gitbook.io/chembl-interface-documentation/frequently-asked-questions/chembl-data-questions#what-is-the-confidence-score) explained what the confidence score meant.
...
bioact_assay_df.head()
```

*Test to relax confidence score limit to include 8 as well as 9 - not running following code block*

```{python}
# 493 cpds with confidence score of 9
# Limited bioact_assay_df to cpds with conf score of 9 only

# bioact_assay_df = bioact_assay_df[bioact_assay_df["confidence_score"] == 9]
# print(bioact_assay_df.shape)
# bioact_assay_df.head()
```
...
While having identified the protein target and obtained the bioactivity data, along with assay data, this step was to link the bioactivity data to the compound data. This could be done by having the ChEMBL IDs available in the bioactivity dataset.
...
Removing any missing entries in the compound data (excluding the "max_phase" column as I might be doing something about it during the training/testing split part in the upcoming model building post - note: "None" entries likely meant they were preclinical molecules so not assigned with a max phase yet).

...
Initially, I tried using all of Pandas' code of merge/join/concat dataframes. They all failed to create the correct final combined dataframe since there were too many rows, with one run actually creating 540 rows! (maximum should be 481 rows). So I looked into different ways, and since there were no other common column for both dataframes, the index column was the only one that correlate both.

I also found out after going back to the previous steps that when I applied the compound pre-processing function from Datamol, the index of the resultant dataframe was changed to start from 1 (rather than zero). Because of this, I tried re-setting the index of dtree_san_df first, then drop the index column, followed by re-setting the index again to ensure it started at zero, and it worked. So that the dtree_san_df would have exactly the same index as dtree_san_df_dm.

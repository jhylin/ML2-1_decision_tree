...
There were 2 different types of data validity comments found, which were "NaN"  and "Outside typical range" (Note: the only compound with "Potential transcirption error" as data validity comment was already filtered out in part 1). So, this meant compounds with comments other than "NaN" would need to be addressed first.
...
Also as a sanity check on the sanitised compounds in dtree_san_df, I just wanted to see if I could parse all compounds in this dataframe (if not, something was obviously wrong and needed to be re-checked before proceeding to next step).
...
I also used Pandas' built-in boxplot in addition to the histogram to show the possible outliers within the pKi values. Clearly, the outliers for pKi values appeared to be above 10.5. I've also decided not to completely remove these outliers due to the dataset itself being not in a Gaussian distribution (which meant they might not be true outliers).
...
```{python}
# Run descriptor calculations on mol_list
...
Check final dataframe to make sure there were 442 rows (index_x and index_y were identical) and increased number of columns (combined columns from both dataframes). So this finally seemed to work.
...
In general, the order of steps could be changed in a more logical order when I looked back at what I did.

<br>

###### **Checklist for ChEMBL compound data pre-processing**

*Please note this list was not exhaustive as this was only a suggestion from my experience working on this post*

Molecular data pre-processing:
- duplicates
- missing values
- salts or mixture

Consistency of structural annotations:
- molecular validity
- molecular sanity
- charge standardisation
- stereochemistry

Reliability of reported experimental values (e.g. activity values like IC50, Ki, EC50 etc.):
- annotated validity (data_validity_comment)
- presence of outliers
- confidence score (assays)
- standard deviation of multiple entries (if applicable)

- Compute 2D molecular descriptors
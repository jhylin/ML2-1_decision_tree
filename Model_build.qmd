...
I did a test-run on fitting a decision tree on the full dataset of 481 compounds. However, there were no training and testing data splits, so potentially the result might be not that useful. So here I wanted to split the collected data by spliting the compounds into two groups, where compounds with max phase of 4 would be the training data, and for the rest of the compounds with max phases of null to 3, would be my testing data.

Fitting the DecisionTreeRegressor() for compounds with max phase of 4, which should be 10 compounds in total(including donepezil and galantamine).
...
There were 466 compounds with max phase as "null".

...
Interestingly, there were 3 different types of data validity comments found, which were "NaN", "Outside typical range" and "Potential transcirption error". So, this meant we would need to address compounds with comments in the latter two and potentially leaving the ones with "NaN" alone.

```{python}
# Find out compounds with "outside typical range" as data validity comment
dtree_df_filter1 = dtree_df[dtree_df["data_validity_comment"] == "Outside typical range"]
dtree_df_filter1
```
...
```{python}
dtree_df_filter2 = dtree_df[dtree_df["data_validity_comment"] == "Potential transcription error"]
dtree_df_filter2
```

There were 8 compounds with potential transcription errors for their respective IC50 values!

This meant it would be the best to remove the above compounds with questionable IC50 values.
...
Checking the dtree_df dataframe again and also whether if only the compounds with zero labelled "data_validity_comment" were kept (and other two types were removed).
...
**Sections below needed to be re-run as only just checked and cleaned data for "data_validity_comment" column just now**
...
def _preprocess(row):
    ...
    # Added a new column for RDKit molecules
...
Please note if the dataset required for sanitisation is large, Datamol suggested using their example code to add parallelisation as shown below.
...
```
In this case, I tried using without adding the parallelisation code, the whole sanitisation process wasn't overly long, and was done within a minute or so (the dtree_df dataframe had 5,039 rows or compounds only).
...
```{python}
# Check what data were available from chembl_webresource_client

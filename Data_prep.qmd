...
**Following section still work-in-progress**

The other thing to do was to convert the IC50 values to pIC50 values (the negative log of IC50 in molar units).

The key to understand this was to treat pIC50 similarly to how we understand pH for our acids and bases. pIC50 was a dimensionless value (so no units actually!) - *useful link*. The formula to convert from IC50 to pIC50 for nM units was:

pIC50 = 9 - log10(IC50)

Import the math library and set up a small function to do the conversion.

```{python}
import math

# Won't work yet - draft function only
def calc_pIC50(IC50):
    pIC50_value = 9 - math.log10(IC50)
    return pIC50_value
```

Applying the calc_pIC50 function to convert all rows of the compound dataset.
...
# Check for the lowest IC50 values in df
dtree_df["IC50"].min()
```
...
Appears there are IC50 with 0 (or less than 1), which means the function to convert IC50 to pIC50 may produce odd negative numbers for less than 1 IC50 values, for zeros, this would produce an error message in math library.

**Need to drop row entries where IC50 were zero nM**
